...
def market_recommendations(market_df, categoria, canal):
    ...
    if m.empty:
        return ["Sin datos suficientes para esa categoría/canal"]
    ...
    if growth > 12:
        recs.append("📈 Categoría en alto crecimiento → oportunidad clara")

    if comp > 8:
        recs.append("🧱 Competencia alta → diferenciar con pack + claim")

    recs.append(f"🏷 Claim dominante: {claim_top}")
    recs.append(f"📦 Pack tendencia: {pack_top}")
    recs.append(f"💰 Banda precio: {p25:.0f} — {p75:.0f}")

    if growth < 0:
        recs.append("⚠️ Categoría decreciendo → validar con piloto")
...
# ----------------------------
# MODO C: Video de anaquel + ROI recortes + MNL
# ----------------------------
st.divider()
st.markdown("## C) Video de anaquel (frame selector + ROIs + ranking MNL)")
st.caption("Sube un video del anaquel. Selecciona un frame y define ROIs igual que en foto.")

video_file = st.file_uploader(
    "Sube video de anaquel (mp4/mov/webm)",
    type=["mp4", "mov", "webm"],
    key="shelf_video"
)

def _read_video_frames_bytes(video_bytes: bytes, max_frames: int = 120):
    """
    Lee frames con imageio (sin OpenCV).
    Devuelve: frames(list PIL), fps(float)
    """
    try:
    ...
    # imageio.v3 puede leer directo desde bytes con extension hint usando uri="<bytes>"
    # workaround: escribir a un tmp file (más robusto en Streamlit Cloud)
    import tempfile
...
if video_file is not None:
    ...
    # Preview del video (Streamlit lo puede mostrar)
    st.video(video_bytes)

    frames, fps = _read_video_frames_bytes(video_bytes, max_frames=140)

    if not frames:
        st.warning("No pude extraer frames. Prueba otro formato (mp4 h264) o video más corto.")
    else:
        ...
        # ROIs igual que en foto
        w, h = frame.size
        st.markdown("### Define recortes (ROIs) en este frame")
        st.caption("Tip: ROI 0 tu pack, ROI 1-3 competidores. Ajusta a ojo.")
        ...
        # Opcional: muestrear varios frames para robustez
        ...
        if st.button("🧲 Calcular Shelf 3s (VIDEO)", key="btn_shelf_video_calc"):
            ...
            # Selecciona frames desde el frame_idx hacia adelante (con stride)
            picks = []
            cur = frame_idx
            ...
            for j, fi in enumerate(picks):
                ...
                for idx, (roi, lab) in enumerate(zip(rois, labels)):
                    ...
                    emotion = 60.0  # proxy (si quieres, aquí metes emoción real)
                    choice = pack_3sec_choice_score(
                        sc["pack_legibility_score"],
                        sc["pack_shelf_pop_score"],
                        sc["pack_clarity_score"],
            ...
            # promedia resultados por pack
            rows2 = []
            for lab in labels:
                vals = rows_accum.get(lab, [])
                if vals:
                    rows2.append({"pack": lab, "choice_3s": round(float(np.mean(vals)), 1)})
                else:
                    rows2.append({"pack": lab, "choice_3s": 0.0})

            rank = shelf_rank_from_pack_scores(rows2)
            st.dataframe(rank, use_container_width=True)

            # Guardar sesión + learning log
            st.session_state.last_shelf = {
                "mode": "shelf_video",
...
def coldstart_recommendations(success_model, sales_model, base_row: dict, top_k: int = 12):
    """
    What-if rápido y estable:
    - 5 precios x 5 márgenes x 4 packProxy x 4 claimsProxy = 400 escenarios (rápido)
    ...
    # Grid conservador (rápido y robusto)
        ...
        for m in margen_grid:
            for dc in claims_proxy_grid:
                for dp in pack_proxy_grid:
                    ...
                    # mejora de conexión proxy (limitada)
    ...
    # ✅ Asegurar salida aunque algo raro pase
    if out.empty:
        out = pd.DataFrame([{
...
# ============================================================
# 🧊 PRODUCTO NUEVO — Cold Start + What-If Recomendaciones (FIX REAL)
# ============================================================
with tab_new:
    ...
    recs = recommend_claims(segmento, canal_norm, 10) if "recommend_claims" in globals() else []
    claim_opts = [c for c, _ in recs] if recs else []
    if not claim_opts:
        st.warning("No hay claims recomendados. Revisa CLAIMS_LIBRARY o recommend_claims().")
        claims_sel = []
    else:
    ...
    cscore = float(claims_score(claims_sel, canal_norm)) if "claims_score" in globals() else 0.0
    st.metric("Claims Score", f"{cscore:.1f}/100")
    ...
    # -----------------------------
    # Comparables p25/p50/p75
    # -----------------------------
    comp = df.copy()
    if "categoria" in df.columns:
        comp2 = df[df["categoria"] == str(categoria).lower()].copy()
    ...
    # ============================================================
    # ✅ WHAT-IF dentro del mismo TAB (esto era el bug)
    # ============================================================
    st.divider()
    ...
    # (Opcional) Debug compacto
    with st.expander("🔎 Debug (opcional)"):
        st.write("base_row:", base_row)
        st.write("coldstart_recommendations existe?:", "coldstart_recommendations" in globals())
    ...
    if st.button("🚀 Generar recomendaciones (what-if)", key="btn_new_recos"):
        ...
        if "coldstart_recommendations" not in globals():
            st.session_state.new_recos_error = "❌ No existe la función coldstart_recommendations()."
        else:
            try:
    ...
    if st.session_state.new_recos_out is not None:
        ...
        st.markdown("### 🧪 Top escenarios (para iterar)")
        show = out_df.copy()
...
# ============================================================
# 📈 MARKET INTELLIGENCE (con subcategoría opcional)
# ============================================================
with tab_market:
    ...
    # --- Detecta columna de subcategoría (si existe con otro nombre)
    ...
    cat = col1.selectbox("Categoría", ["Todas"] + categorias, key="mk_cat_v3")
    canal = col2.selectbox("Canal", ["Todos"] + canales, key="mk_canal_v3")
    ...
    if canal != "Todos":
        dfm = dfm[dfm["canal"].astype(str) == str(canal)]

    # Subcategoría SOLO si existe columna
    if sub_col:
        subcats = (
            dfm[sub_col]
            .dropna()
        ...
        sub = col3.selectbox("Subcategoría", ["Todas"] + subcats, key="mk_sub_v3")

        if sub != "Todas":
            dfm = dfm[dfm[sub_col].astype(str) == str(sub)]
    else:
        col3.info("ℹ️ Tu market_intel.csv no trae columna 'subcategoria'. (Filtro desactivado)")

    st.divider()

    # -----------------------------
    # Vista tabla (elige columnas si existen)
    # -----------------------------
    prefer_cols = [
    ...
    if not show_cols:
        ...
        st.dataframe(dfm[show_cols].head(200), use_container_width=True)

    # -----------------------------
    # Recomendaciones (robustas)
    # -----------------------------
    ...
    if growth is not None:
        if growth > 12:
            recs.append("📈 Crecimiento alto → oportunidad clara de entrada / expansión.")
        elif growth < 0:
            recs.append("⚠️ Crecimiento negativo → validar con piloto antes de escalar.")
    if comp is not None and comp > 8:
        recs.append("🧱 Competencia alta → necesitas diferenciador fuerte (pack + claim + precio).")
    if p25 is not None and p75 is not None:
        recs.append(f"💰 Banda de precio sugerida (p25–p75): ${p25:.0f} — ${p75:.0f}")
    if claim_top and claim_top != "(sin dato)":
        recs.append(f"🏷 Claim/tema dominante: {claim_top}")
    if pack_top and pack_top != "(sin dato)":
        recs.append(f"📦 Estilo de pack tendencia: {pack_top}")

    if not recs:
        recs = ["No hay suficientes señales en el dataset para recomendar (revisa columnas/valores)."]
    ...
    with st.expander("🧩 Ver columnas detectadas"):
        st.write(dfm.columns.tolist())
        st.write("Subcol detectada:", sub_col)
