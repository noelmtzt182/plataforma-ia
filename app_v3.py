...
with tab_market:
    ...
    if market_df is None:
        st.warning("No existe market_intel.csv — súbelo o colócalo en el repo.")
        st.stop()

    if market_df.empty:
        st.warning("market_intel.csv está vacío.")
        st.stop()
    ...
    Can = st.selectbox(
        "Canal",
        sorted(market_df["can"].dropna().unique()),
        key="mi_can_v3"
    )
    Sub = st.selectbox(
        "Subcategoria",
        sorted(market_df["sub"].dropna().unique()),
        key="mi_sub_v3"
    )
    sub = market_df[
