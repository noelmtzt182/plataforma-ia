...
with tab_market:
    ...
    if market_df is None or market_df.empty:
        ...
        categorias = sorted(market_df["categoria"].dropna().unique().tolist())
        canales = sorted(market_df["canal"].dropna().unique().tolist())
        subcats = sorted(market_df["subcategoria"].dropna().unique().tolist())

        cat = col1.selectbox("Categoría", ["Todas"] + categorias, key="mk_cat")
        canal = col2.selectbox("Canal", ["Todos"] + canales, key="mk_canal")
        ...
        if canal != "Todos":
            dfm = dfm[dfm["canal"] == canal]

        if sub != "Todas":
            dfm = dfm[dfm["subcategoria"] == sub]
        ...
        top_trends = (
            dfm.sort_values("tendencia", ascending=False)
               .head(5)
        )

        for _, r in top_trends.iterrows():
            st.write(
                f"• **{r['subcategoria']}** en **{r['canal']}** "
                f"muestra alta tendencia. Claim dominante: "
                f"**{r['claim_top']}**."
            )
