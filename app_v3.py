...
with tab_new:
    ...
    # ============================================================
    # ✅ FIX: What-If persistente (siempre muestra resultados)
    # ============================================================
    st.divider()
    st.markdown("## 🧠 Recomendaciones What-If (Producto Nuevo)")
    st.caption("Genera escenarios variando precio/margen + mejoras proxy (pack/claims) para subir probabilidad y ventas. "
               "Los resultados se quedan visibles aunque Streamlit haga rerun.")

    with st.expander("Abrir recomendaciones", expanded=True):
        # Debug mínimo para confirmar inputs base
        st.write("**Base inputs:**", {
            "precio": float(precio),
            "margen_pct": float(margen),
            "canal": canal_norm,
            "conexion_score": float(conexion_score),
        ...
        if st.button("🚀 Generar recomendaciones (what-if)", key="new_recos_btn_fix"):
            try:
                base_row = {
                    "precio": float(precio),
                    "competencia": float(competencia),
        ...
        # Mostrar si ya existen resultados
        if st.session_state.new_recos_out is None:
            st.info("Presiona el botón para generar escenarios.")
        else:
            out_df = st.session_state.new_recos_out
            recs_txt = st.session_state.new_recos_txt or []
            summary = st.session_state.new_recos_sum or {}

            if isinstance(out_df, pd.DataFrame) and out_df.empty:
                st.warning("No se generaron escenarios (out_df vacío). Revisa que el modelo esté entrenado y que tus inputs sean válidos.")
            else:
                cA, cB, cC = st.columns(3)
                cA.metric("Prob base", f"{summary.get('base_prob_%', 0):.1f}%")
                cB.metric("Mejor prob", f"{summary.get('best_prob_%', 0):.1f}%")
                cC.metric("Uplift", f"+{summary.get('uplift_prob_pp', 0):.1f} pp")
                ...
                if not recs_txt:
                    st.write("• No hubo quick wins detectados. Prueba variar precio/margen o subir pack/anaquel para mejorar señales.")
                else:
                    for r in recs_txt:
                        st.write("•", r)
                ...
                cols_show = [c for c in [
                    ...
                    "delta_claims_proxy", "delta_pack_proxy", "ventas_unidades", "uplift_sales"
