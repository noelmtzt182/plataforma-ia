...
with tab_sim:
    ...
    selected_claims = st.multiselect("Selecciona claims (ideal 2-3)", claim_options, default=claim_options[:2], key="sim_claims")
...
with tab_ins:
    ...
    with d1:
        ...
        dist_plot = dist.reset_index()
        dist_plot.columns = ["bucket", "count"]
        st.bar_chart(dist_plot.set_index("bucket"), use_container_width=True)
    ...
    with d2:
        ...
        dist2_plot = dist2.reset_index()
        dist2_plot.columns = ["bucket", "count"]
        st.bar_chart(dist2_plot.set_index("bucket"), use_container_width=True)
...
with tab_pack:
    ...
    if img_file is None:
        ...
        if scores["pack_legibility_score"] < 60:
            recs.append("• Sube legibilidad: más contraste texto/fondo, tipografía más gruesa y menos elementos alrededor del claim principal.")
        if scores["pack_clarity_score"] < 60:
            recs.append("• Mejora claridad: reduce ruido visual, deja aire y limita a 2–3 claims.")
        if scores["pack_shelf_pop_score"] < 60:
            recs.append("• Sube shelf pop: usa color acento y evita pack muy oscuro o lavado.")
        if m["edge_density"] > 0.28:
            recs.append("• Saturación visual alta: simplifica fondos, patrones y microtextos.")
        if not recs:
            recs.append("• Va bien visualmente. Ajusta jerarquía: Marca → beneficio → variedad/sabor → prueba/credencial.")
        st.write("\n".join(recs))
...
with tab_claims:
    ...
    canal = c2.selectbox("Canal", ["retail","marketplace"], 0, key="claims_canal")
...
with tab_exp:
    ...
    marca = v1.selectbox("Marca", sorted(df["marca"].unique().tolist()), 0, key="exp_marca")
    canal = v2.selectbox("Canal", sorted(df["canal"].unique().tolist()), 0, key="exp_canal")
