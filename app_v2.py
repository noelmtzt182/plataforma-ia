# app_v2.py
# ============================================================
# Plataforma IA: Producto + Empaque + Claims + Shelf 3-Second Test (v2.0)
# Incluye:
# ✅ Vista inversionista (TAM/escenarios/unit economics/score lanzamiento)
# ✅ Motor recomendaciones (what-if optimizer)
# ✅ Pack Vision+ (imagen -> métricas -> recomendaciones -> emoción)
# ✅ Reporte ejecutivo descargable (TXT + CSV inputs)
# ✅ Shelf & Emotion Predictor (3-Second Test) v2.0:
#    - comparación vs competidores
#    - heatmap visual (ligero)
#    - simulación multinomial (softmax)
#    - learning con resultados reales (logistic regression)
# ============================================================
...
def pack_scores_from_metrics(m: dict) -> dict:
    ...
    # Extras para Shelf module (aproximaciones estables)
...
# ----------------------------
# Claims engine (base + data-driven simple)
# ----------------------------
CLAIMS_LIBRARY = {
    "fit": [
...
CANAL_CLAIM_BOOST = {
    "retail": {"Sin azúcar añadida": 1.04, "Alto en fibra": 1.03, "Integral": 1.02, "Ideal para la familia": 1.02},
    "marketplace": {"Alto en proteína": 1.05, "Sin colorantes artificiales": 1.04, "Ingredientes seleccionados": 1.03},
}

# “Librería extendida” de claims (genérica, para demo; tú luego la vuelves por categoría)
EXTRA_CLAIMS = [
    ("Sin gluten", 0.76),
...
# ----------------------------
# Conexión final (producto) = rating + sentiment + uplift pack/claims
# ----------------------------
...
uploaded = st.sidebar.file_uploader("Sube tu CSV (con ventas)", type=["csv"], key="uploader_csv_v2")
...
st.caption("Éxito + Ventas estimadas + Insights + Pack Vision+ + Claims + Experimentos + 3-Second Test v2.0")
...
# Tabs
# ----------------------------
tab_sim, tab_ins, tab_pack, tab_claims, tab_exp, tab_inv, tab_rec, tab_report, tab_data, tab_model, tab_shelf = st.tabs(
    [
        "🧪 Simulador",
        "📊 Insights",
        "📦 Pack Lab",
...
with tab_sim:
    ...
    marca = c1.selectbox("Marca", marcas, 0, key="sim_marca_v2")
    canal = c2.selectbox("Canal", canales, 0, key="sim_canal_v2")
    segmento = c3.selectbox("Segmento objetivo", ["fit", "kids", "premium", "value"], 0, key="sim_segmento_v2")

    st.markdown("### Variables de negocio")
    b1, b2, b3, b4, b5 = st.columns(5)
    precio = b1.number_input("Precio", value=float(df["precio"].median()), step=1.0, key="sim_precio_v2")
    competencia = b2.slider("Competencia (1-10)", 1, 10, int(df["competencia"].median()), key="sim_competencia_v2")
    demanda = b3.slider("Demanda (10-100)", 10, 100, int(df["demanda"].median()), key="sim_demanda_v2")
    tendencia = b4.slider("Tendencia (20-100)", 20, 100, int(df["tendencia"].median()), key="sim_tendencia_v2")
    margen_pct = b5.slider("Margen %", 0, 90, int(np.clip(df["margen_pct"].median(), 0, 90)), key="sim_margen_pct_v2")

    st.markdown("### Empaque + Claims")
    p1, p2, p3 = st.columns(3)
    pack_legibility_score = p1.slider("Pack legibilidad (0-100)", 0, 100, 65, key="sim_pack_leg_v2")
    pack_shelf_pop_score = p2.slider("Pack shelf pop (0-100)", 0, 100, 70, key="sim_pack_pop_v2")
    pack_clarity_score = p3.slider("Pack claridad (0-100)", 0, 100, 65, key="sim_pack_clarity_v2")
    ...
    copy = st.text_input("Copy corto (opcional)", value="Energía y nutrición para tu día", key="sim_copy_v2")
    ...
    rating_conexion = st.slider("Rating conexión producto (1-10)", 1, 10, 7, key="sim_rating_v2")
    sentiment_score = st.select_slider("Sentimiento del producto (-1/0/1)", options=[-1, 0, 1], value=1, key="sim_sentiment_v2")
    ...
    if st.button("🚀 Simular", key="sim_btn_v2"):
...
with tab_pack:
    ...
    img_file = st.file_uploader("Sube imagen del empaque (PNG/JPG)", type=["png", "jpg", "jpeg"], key="pack_uploader_v2")
    if img_file is None:
...
with tab_claims:
    ...
    c1, c2 = st.columns(2)
    seg = c1.selectbox("Segmento", ["fit", "kids", "premium", "value"], 0, key="claims_segmento_v2")
    canal_c = c2.selectbox("Canal", ["retail", "marketplace"], 0, key="claims_canal_v2")
...
with tab_exp:
    ...
    c1, c2, c3 = st.columns(3)
    exp_name = c1.text_input("Nombre experimento", value="Test Pack v1 vs v2", key="exp_name_v2")
    variant = c2.selectbox("Variante", ["A", "B"], 0, key="exp_variant_v2")
    metric = c3.selectbox("Métrica", ["intencion_compra", "conexion_pack", "ventas_piloto"], 0, key="exp_metric_v2")

    v1, v2, v3 = st.columns(3)
    marca_e = v1.selectbox("Marca", sorted(df["marca"].unique().tolist()), 0, key="exp_marca_v2")
    canal_e = v2.selectbox("Canal", sorted(df["canal"].unique().tolist()), 0, key="exp_canal_v2")
    value = v3.number_input("Valor observado", value=7.0, step=0.1, key="exp_value_v2")

    if st.button("➕ Guardar medición", key="exp_save_v2"):
...
with tab_inv:
    ...
    tam = colA.number_input("TAM (MXN)", value=8_000_000_000, step=100_000_000, key="inv_tam")
    som_pct = colB.slider("SOM % (capturable)", 0.1, 10.0, 1.5, 0.1, key="inv_som")
    asp_pct = colC.slider("Aspiracional % (3 años)", 0.1, 20.0, 4.0, 0.1, key="inv_asp")
    gross_margin = colD.slider("Margen bruto %", 10, 80, 45, key="inv_gm")

    # Scenarios
    st.markdown("### Escenarios (Revenue potencial)")
    s1, s2, s3 = st.columns(3)
    ...
    price = u1.number_input("Precio promedio (MXN)", value=120.0, step=1.0, key="inv_price")
    cogs = u2.number_input("COGS (MXN)", value=70.0, step=1.0, key="inv_cogs")
    cac = u3.number_input("CAC por cliente (MXN)", value=25_000.0, step=1000.0, key="inv_cac")
    arpa = u4.number_input("ARPA anual (MXN)", value=150_000.0, step=5000.0, key="inv_arpa")
    ...
    if lr:
        score_lanz = (
            ...
            0.25 * (pack.get("choice", 50)) +
            0.15 * (lr.get("claims_score", 50)) +
            0.15 * (lr.get("pack_emotion_score", 50))
        )
    st.metric("Launch Score (0–100)", f"{score_lanz:.1f}")
...
with tab_rec:
    ...
    st.caption("Busca el mejor combo tipo 'what-if' (ligero) sobre tu escenario actual.")
    ...
    if not lr:
        ...
        base_claims = lr.get("claims", [])
        base_pack_emotion = float(lr.get("pack_emotion_score", 60))

        c1, c2, c3 = st.columns(3)
        price_range = c1.slider("Rango precio (±%)", 0, 40, 15, key="opt_price_rng")
        margin_range = c2.slider("Rango margen (± puntos)", 0, 30, 10, key="opt_margin_rng")
        tries = c3.slider("Iteraciones", 50, 400, 200, 50, key="opt_tries")
        ...
        for _ in range(int(tries)):
            ...
            # claims: elige 2-3 al azar
            k = int(rng.integers(2, 4))
            claims_try = list(rng.choice(claim_pool, size=k, replace=False))

            cscore = claims_score(claims_try, canal)

            # pack emotion proxy: si subiste pack, usamos choice del shelf_pred como señal
...
with tab_report:
    ...
    report_txt = "\n".join(lines)
    st.text_area("Vista previa", report_txt, height=280, key="rep_preview")
    ...
    # CSV con inputs (si existen)
    inputs = {}
    if lr:
        inputs.update({k: lr.get(k) for k in lr.keys()})
    if lp.get("pack_scores"):
...
# ============================================================
with tab_shelf:
    st.subheader("👀 Shelf & Emotion Predictor (3-Second Test) — v2.0")

    st.caption(
        "Sube tu pack y competidores. El sistema estima: Atención, Emoción, Recordación y Elección. "
        "Incluye heatmap, simulación multinomial y aprendizaje con resultados reales."
    )
    ...
    files = st.file_uploader(
        ...
        key="shelf_multi_upload_v2",
    )

    show_heat = st.checkbox("Mostrar heatmap (saliency) por pack", value=True, key="shelf_show_heat_v2")
    alpha = st.slider("Intensidad heatmap", 0.15, 0.75, 0.45, 0.05, key="shelf_heat_alpha_v2")
    ...
    if not files or len(files) < 2:
        ...
        sel = st.selectbox("Selecciona pack para ver detalle", names, index=0, key="shelf_sel_pack_v2")
        ...
        # Learning con resultados reales
        st.markdown("### 🧪 Learning con resultados reales (feedback loop)")
        st.caption("Registra quién ganó en un test real (A/B, encuesta o piloto). Con el tiempo, el modelo aprende.")

        winner = st.selectbox("¿Cuál pack fue elegido (ganador real)?", options=names, index=0, key="shelf_winner_select_v2")
        context = st.text_input("Contexto (opcional): canal / tienda / público / fecha", value="", key="shelf_context_v2")

        if st.button("➕ Guardar resultado real", key="shelf_save_label_v2"):
            for row in results:
