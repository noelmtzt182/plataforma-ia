# app.py
# ============================================================
# Plataforma IA: Producto + Empaque + Claims + Shelf 3-Second Test (v2.1)
# Incluye:
...
# ✅ NUEVO: 🧊 Producto Nuevo (Cold Start) con comparables + rango p25/p50/p75 + score + GO/AJUSTAR/NO-GO
# ============================================================
...
uploaded = st.sidebar.file_uploader("Sube tu CSV (con ventas)", type=["csv"], key="uploader_csv_v21")
...
with tab_sim:
    ...
    marca = c1.selectbox("Marca", marcas, 0, key="sim_marca_v21")
    canal = c2.selectbox("Canal", canales, 0, key="sim_canal_v21")
    segmento = c3.selectbox("Segmento objetivo", ["fit", "kids", "premium", "value"], 0, key="sim_segmento_v21")

    st.markdown("### Variables de negocio")
    b1, b2, b3, b4, b5 = st.columns(5)
    precio = b1.number_input("Precio", value=float(df["precio"].median()), step=1.0, key="sim_precio_v21")
    competencia = b2.slider("Competencia (1-10)", 1, 10, int(df["competencia"].median()), key="sim_comp_v21")
    demanda = b3.slider("Demanda (10-100)", 10, 100, int(df["demanda"].median()), key="sim_dem_v21")
    tendencia = b4.slider("Tendencia (20-100)", 20, 100, int(df["tendencia"].median()), key="sim_tend_v21")
    margen_pct = b5.slider("Margen %", 0, 90, int(np.clip(df["margen_pct"].median(), 0, 90)), key="sim_margen_v21")

    st.markdown("### Empaque + Claims")
    p1, p2, p3 = st.columns(3)
    pack_legibility_score = p1.slider("Pack legibilidad (0-100)", 0, 100, 65, key="sim_pack_leg_v21")
    pack_shelf_pop_score = p2.slider("Pack shelf pop (0-100)", 0, 100, 70, key="sim_pack_pop_v21")
    pack_clarity_score = p3.slider("Pack claridad (0-100)", 0, 100, 65, key="sim_pack_cla_v21")
    ...
    copy = st.text_input("Copy corto (opcional)", value="Energía y nutrición para tu día", key="sim_copy_v21")
    ...
    rating_conexion = st.slider("Rating conexión producto (1-10)", 1, 10, 7, key="sim_rating_v21")
    sentiment_score = st.select_slider("Sentimiento del producto (-1/0/1)", options=[-1, 0, 1], value=1, key="sim_sent_v21")
    ...
    if st.button("🚀 Simular", key="sim_btn_v21"):
...
with tab_pack:
    ...
    img_file = st.file_uploader("Sube imagen del empaque (PNG/JPG)", type=["png", "jpg", "jpeg"], key="pack_uploader_v21")
    if img_file is None:
...
with tab_claims:
    ...
    c1, c2 = st.columns(2)
    seg = c1.selectbox("Segmento", ["fit", "kids", "premium", "value"], 0, key="claims_segmento_v21")
    canal_c = c2.selectbox("Canal", ["retail", "marketplace"], 0, key="claims_canal_v21")
...
with tab_exp:
    ...
    c1, c2, c3 = st.columns(3)
    exp_name = c1.text_input("Nombre experimento", value="Test Pack v1 vs v2", key="exp_name_v21")
    variant = c2.selectbox("Variante", ["A", "B"], 0, key="exp_variant_v21")
    metric = c3.selectbox("Métrica", ["intencion_compra", "conexion_pack", "ventas_piloto"], 0, key="exp_metric_v21")

    v1, v2, v3 = st.columns(3)
    marca_e = v1.selectbox("Marca", sorted(df["marca"].unique().tolist()), 0, key="exp_marca_v21")
    canal_e = v2.selectbox("Canal", sorted(df["canal"].unique().tolist()), 0, key="exp_canal_v21")
    value = v3.number_input("Valor observado", value=7.0, step=0.1, key="exp_value_v21")

    if st.button("➕ Guardar medición", key="exp_save_v21"):
...
with tab_inv:
    ...
    tam = colA.number_input("TAM (MXN)", value=8_000_000_000, step=100_000_000, key="inv_tam_v21")
    som_pct = colB.slider("SOM % (capturable)", 0.1, 10.0, 1.5, 0.1, key="inv_som_v21")
    asp_pct = colC.slider("Aspiracional % (3 años)", 0.1, 20.0, 4.0, 0.1, key="inv_asp_v21")
    gross_margin = colD.slider("Margen bruto %", 10, 80, 45, key="inv_gm_v21")
    ...
    price = u1.number_input("Precio promedio (MXN)", value=120.0, step=1.0, key="inv_price_v21")
    cogs = u2.number_input("COGS (MXN)", value=70.0, step=1.0, key="inv_cogs_v21")
    cac = u3.number_input("CAC por cliente (MXN)", value=25_000.0, step=1000.0, key="inv_cac_v21")
    arpa = u4.number_input("ARPA anual (MXN)", value=150_000.0, step=5000.0, key="inv_arpa_v21")
...
with tab_rec:
    ...
    if not lr:
        ...
        c1, c2, c3 = st.columns(3)
        price_range = c1.slider("Rango precio (±%)", 0, 40, 15, key="opt_price_rng_v21")
        margin_range = c2.slider("Rango margen (± puntos)", 0, 30, 10, key="opt_margin_rng_v21")
        tries = c3.slider("Iteraciones", 50, 400, 200, 50, key="opt_tries_v21")
...
with tab_shelf:
    ...
    files = st.file_uploader(
        ...
        key="shelf_multi_upload_v21",
    )

    show_heat = st.checkbox("Mostrar heatmap (saliency) por pack", value=True, key="shelf_show_heat_v21")
    alpha = st.slider("Intensidad heatmap", 0.15, 0.75, 0.45, 0.05, key="shelf_heat_alpha_v21")
    ...
    if not files or len(files) < 2:
        ...
        sel = st.selectbox("Selecciona pack para ver detalle", names, index=0, key="shelf_sel_pack_v21")
        ...
        winner = st.selectbox("¿Cuál pack fue elegido (ganador real)?", options=names, index=0, key="shelf_winner_select_v21")
        context = st.text_input("Contexto (opcional): canal / tienda / público / fecha", value="", key="shelf_context_v21")

        if st.button("➕ Guardar resultado real", key="shelf_save_label_v21"):
            for row in results:
...
# ============================================================
# 🧊 Producto Nuevo — Cold Start Predictor (INTEGRADO)
# ============================================================
with tab_new:
    ...
    categoria = c1.selectbox("Categoría comparable", categorias, key="new_cat_v21")
    canal = c2.selectbox("Canal", canales, key="new_canal_v21")
    segmento = c3.selectbox("Segmento objetivo", ["fit", "kids", "premium", "value"], key="new_seg_v21")

    b1, b2, b3, b4, b5 = st.columns(5)
    precio = b1.number_input("Precio", 1.0, 9999.0, float(df["precio"].median()), step=1.0, key="new_precio_v21")
    margen = b2.slider("Margen %", 0, 90, int(np.clip(df["margen_pct"].median(), 0, 90)), key="new_margen_v21")
    competencia = b3.slider("Competencia (1-10)", 1, 10, int(df["competencia"].median()), key="new_comp_v21")
    demanda = b4.slider("Demanda (10-100)", 10, 100, int(df["demanda"].median()), key="new_dem_v21")
    tendencia = b5.slider("Tendencia (20-100)", 20, 100, int(df["tendencia"].median()), key="new_tend_v21")

    # Claims
    st.markdown("### 🏷️ Claims (cold start)")
    recs = recommend_claims(segmento, canal, 10)
    claim_opts = [c for c, _ in recs]
    claims_sel = st.multiselect("Selecciona 2-3 claims", claim_opts, default=claim_opts[:2], key="new_claims_v21")
    cscore = claims_score(claims_sel, canal)
    st.metric("Claims Score", f"{cscore:.1f}/100")

    # Pack (imagen)
    st.markdown("### 📦 Empaque (opcional, recomendado)")
    img = st.file_uploader("Sube empaque (PNG/JPG)", type=["png","jpg","jpeg"], key="new_pack_v21")
    ...
    # Entrada para modelos (marca nueva se maneja con OneHotEncoder ignore)
    entrada = pd.DataFrame([{
        "precio": float(precio),
        "competencia": float(competencia),
        "demanda": float(demanda),
    ...
    # Comparables (top 20 por distancia)
...
with tab_report:
    ...
    report_txt = "\n".join(lines)
    st.text_area("Vista previa", report_txt, height=320, key="rep_preview_v21")
