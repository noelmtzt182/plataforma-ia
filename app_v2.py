...
with tab_sim:
    ...
    marca = c1.selectbox("Marca", marcas)
    canal = c2.selectbox("Canal", canales)
    segmento = c3.selectbox("Segmento", ["fit","kids","premium","value"])

    canal_norm = str(canal).lower().strip()

    b1,b2,b3,b4,b5 = st.columns(5)
    precio = b1.number_input("Precio", value=float(df["precio"].median()))
    competencia = b2.slider("Competencia",1,10,int(df["competencia"].median()))
    demanda = b3.slider("Demanda",10,100,int(df["demanda"].median()))
    tendencia = b4.slider("Tendencia",20,100,int(df["tendencia"].median()))
    margen_pct = b5.slider("Margen %",0,90,int(df["margen_pct"].median()))

    # ---------- Claims ----------
    recs = recommend_claims(segmento, canal_norm, 8)
    claim_opts = [c for c,_ in recs]
    claims = st.multiselect("Claims", claim_opts, claim_opts[:2])
    cscore = claims_score(claims, canal_norm)

    # ---------- Pack ----------
    p1,p2,p3 = st.columns(3)
    pack_leg = p1.slider("Pack legibilidad",0,100,65)
    pack_pop = p2.slider("Pack pop",0,100,70)
    pack_cla = p3.slider("Pack claridad",0,100,65)
...
with tab_claims:
    ...
    seg = st.selectbox("Segmento", ["fit","kids","premium","value"])
    can = st.selectbox("Canal", ["retail","marketplace"])
    ...
    sel = st.multiselect("Selecciona claims", [c for c,_ in recs])
...
with tab_pack:
    ...
    img_file = st.file_uploader("Sube pack", type=["png","jpg","jpeg"])
...
with tab_new:
    ...
    precio = st.number_input("Precio",100.0)
    margen = st.slider("Margen",0,90,30)
...
with tab_market:
    ...
    if market_df is not None:
        st.dataframe(market_df.head(100))
    else:
...
with tab_report:
    ...
    st.download_button("Descargar TXT", txt)
