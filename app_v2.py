...
# ============================================================
with tab_new:
    st.subheader("🧊 Producto Nuevo — Cold Start (sin histórico) + Recomendaciones")
    st.caption("Predice éxito y ventas usando atributos + claims + pack (opcional) + comparables aproximados.")

    c1,c2,c3 = st.columns(3)
    categorias = sorted(df["categoria"].unique().tolist())
    canales = sorted(df["canal"].unique().tolist())

    categoria = c1.selectbox("Categoría comparable", categorias, key="new_categoria")
    canal = c2.selectbox("Canal", canales, key="new_canal")
    segmento = c3.selectbox("Segmento", ["fit","kids","premium","value"], key="new_segmento")
    ...
    recs = recommend_claims(segmento, str(canal).lower(), 10)
    claim_opts = [c for c,_ in recs]
    claims_sel = st.multiselect("Selecciona 2-3 claims", claim_opts, default=claim_opts[:2], key="new_claims")
    cscore = claims_score(claims_sel, str(canal).lower())
    st.metric("Claims Score", f"{cscore:.1f}/100")
    ...
    if img:
        ...
        m = image_metrics(im)
        sc = pack_scores_from_metrics(m)
        pack_wins = pack_quick_wins(sc, m)
        pack_emotion = pack_emotion_score(sc["pack_legibility_score"], sc["pack_shelf_pop_score"], sc["pack_clarity_score"], cscore, 0)
        pack_choice = pack_3sec_choice_score(sc["pack_legibility_score"], sc["pack_shelf_pop_score"], sc["pack_clarity_score"], pack_emotion)

        k1,k2,k3,k4 = st.columns(4)
        k1.metric("Elección (3s)", f"{pack_choice:.1f}/100")
        k2.metric("Emoción (proxy)", f"{pack_emotion:.1f}/100")
    ...
    launch_score = 0.45*(prob*100) + 0.25*pack_choice + 0.15*cscore + 0.15*pack_emotion
    ...
    st.session_state.last_new = {
        "categoria": categoria, "canal": canal, "segmento": segmento,
    ...
    st.divider()
    st.markdown("## 🧠 Recomendaciones para subir probabilidad de éxito (What-if)")
    ...
    if st.button("🚀 Generar recomendaciones (what-if)", key="new_recos_btn"):
        ...
        cC.metric("Uplift", f"+{summary['uplift_prob_pp']:.1f} pp")
        ...
        st.markdown("### 🧪 Top escenarios sugeridos")
        st.dataframe(
